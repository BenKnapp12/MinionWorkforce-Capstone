...
var chunk_VWZ4AVHN_exports = {};
__export(chunk_VWZ4AVHN_exports, {
...
module.exports = __toCommonJS(chunk_VWZ4AVHN_exports);
...
async function formatSchema({ schemas }, inputFormattingOptions) {
  if (process.env.FORCE_PANIC_PRISMA_SCHEMA) {
    handleFormatPanic(() => {
      import_chunk_GI6AU3SG.prismaSchemaWasm.debug_panic();
    });
  }
  ...
  const { formattedMultipleSchemas, lintDiagnostics } = handleFormatPanic(() => {
    ...
    const lintDiagnostics2 = (0, import_chunk_FEUTTZEI.lintSchema)({ schemas: formattedMultipleSchemas2 });
    return { formattedMultipleSchemas: formattedMultipleSchemas2, lintDiagnostics: lintDiagnostics2 };
  });
  const lintWarnings = (0, import_chunk_FEUTTZEI.getLintWarningsAsText)(lintDiagnostics);
...
function formatWasm(schema, documentFormattingParams) {
  const formattedSchema = import_chunk_GI6AU3SG.prismaSchemaWasm.format(schema, JSON.stringify(documentFormattingParams));
...
async function processSchemaResult({
  ...
  const configFromPsl = await (0, import_chunk_JQMDV4CM.getConfig)({ datamodel: schemaResult.schemas, ignoreEnvVarErrors });
...
async function getGenerators(options) {
  ...
  const dmmf = await (0, import_chunk_5ITRR2LK.getDMMF)({
...
You need to define \`output\` in the generator block in the schema file.`
        ...
        const datamodel = (0, import_chunk_HVF6BZ24.mergeSchemas)({ schemas: schemaContext.schemaFiles });
    ...
    for (const generator of generators) {
      if (generator.manifest && generator.manifest.requiresEngines) {
        ...
        if (engineVersion !== version2 && generator.options && generator.manifest.requiresEngines.includes(queryEngineType) && generatorBinaryPaths[queryEngineType] && generatorBinaryPaths[queryEngineType]?.[binaryTarget]) {
          const customDmmf = await (0, import_chunk_5ITRR2LK.getDMMF)({
            datamodel: schemaContext.schemaFiles,
            previewFeatures
          });
...
function checkUnsupportedDataProxy({
  ...
  const url = (0, import_chunk_JQMDV4CM.resolveUrl)((0, import_chunk_JQMDV4CM.getEffectiveUrl)(schemaContext.primaryDatasource));
