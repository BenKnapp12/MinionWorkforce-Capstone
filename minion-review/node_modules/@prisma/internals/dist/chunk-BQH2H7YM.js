...
var chunk_BQH2H7YM_exports = {};
__export(chunk_BQH2H7YM_exports, {
...
module.exports = __toCommonJS(chunk_BQH2H7YM_exports);
...
var import_chunk_ZYJQD2CH = require("./chunk-ZYJQD2CH.js");
var import_chunk_ERAGRX7O = require("./chunk-ERAGRX7O.js");
var import_chunk_MM3X7Q65 = require("./chunk-MM3X7Q65.js");
var import_chunk_EYKBXO4E = require("./chunk-EYKBXO4E.js");
...
async function formatSchema({ schemas }, inputFormattingOptions) {
  ...
  const { formattedMultipleSchemas, lintDiagnostics } = handleFormatPanic(() => {
    ...
    const lintDiagnostics2 = (0, import_chunk_MM3X7Q65.lintSchema)({ schemas: formattedMultipleSchemas2 });
    return { formattedMultipleSchemas: formattedMultipleSchemas2, lintDiagnostics: lintDiagnostics2 };
  });
  const lintWarnings = (0, import_chunk_MM3X7Q65.getLintWarningsAsText)(lintDiagnostics);
...
function formatWasm(schema, documentFormattingParams) {
  const formattedSchema = import_chunk_AYOS4YC3.prismaSchemaWasm.format(schema, JSON.stringify(documentFormattingParams));
...
async function processSchemaResult({
  ...
  const configFromPsl = await (0, import_chunk_ZYJQD2CH.getConfig)({ datamodel: schemaResult.schemas, ignoreEnvVarErrors });
...
async function getGenerators(options) {
  ...
  const dmmf = await (0, import_chunk_ERAGRX7O.getDMMF)({
...
You need to define \`output\` in the generator block in the schema file.`
        ...
        const datamodel = (0, import_chunk_EYKBXO4E.mergeSchemas)({ schemas: schemaContext.schemaFiles });
    ...
    for (const generator of generators) {
      if (generator.manifest && generator.manifest.requiresEngines) {
        ...
        if (engineVersion !== version2 && generator.options && generator.manifest.requiresEngines.includes(queryEngineType) && generatorBinaryPaths[queryEngineType] && generatorBinaryPaths[queryEngineType]?.[binaryTarget]) {
          const customDmmf = await (0, import_chunk_ERAGRX7O.getDMMF)({
            datamodel: schemaContext.schemaFiles,
            previewFeatures
          });
...
function checkUnsupportedDataProxy({
  ...
  const url = (0, import_chunk_ZYJQD2CH.resolveUrl)((0, import_chunk_ZYJQD2CH.getEffectiveUrl)(schemaContext.primaryDatasource));
